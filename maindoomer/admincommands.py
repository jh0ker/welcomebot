...
def duelstatus(update: Update, context: CallbackContext):
    ...
    def _handle_limits():
        ...
        # Remove limits
        if arg == 'none':
            run_query('UPDATE chattable set duelmaximum=NULL WHERE chat_id=(?)',
                      (update.effective_chat.id,))
            reply = 'Был убран лимит дуэлей.'
        # Get current status if no argument was given
        elif arg is None:
            status = run_query('SELECT duelmaximum from chattable WHERE chat_id=(?)',
                               (update.effective_chat.id,))
            # If nothing, means no limit
            if not status:
                reply = 'Лимита на дуэли нет.'
            else:
                duelsused = run_query('SELECT duelcount from "chattable" WHERE chat_id=(?)',
                                      (update.effective_chat.id,))
        ...
        # Set maximum
            ...
            try:
                arg = int(arg)
                run_query('UPDATE chattable SET duelmaximum=(?) WHERE chat_id=(?)',
...
def immune(update: Update, context: CallbackContext):
    ...
    if update.effective_message.reply_to_message is not None:
        ...
        run_query('INSERT OR IGNORE INTO exceptions (user_id, chat_id, firstname) VALUES '
                  '(?, ?, ?)', (targetdata.id, update.effective_chat.id, targetdata.first_name))
        # If the user has a username
        if targetdata.username:
            run_query('UPDATE exceptions SET username=(?) WHERE chat_id=(?) AND user_id=(?)',
...
def unimmune(update: Update, context: CallbackContext):
    ...
    if update.effective_message.reply_to_message:
        targetdata = update.effective_message.reply_to_message.from_user
        run_query('DELETE FROM exceptions WHERE user_id=(?) AND chat_id=(?)',
